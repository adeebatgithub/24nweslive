...
class AdminAdvertisementController extends Controller
{
    ...
    public function home_ad_update(Request $request)
    {
        ...
        if($request->hasFile('above_search_ad1')) {
            $request->validate([
                'above_search_ad1' => 'image|mimes:jpg,jpeg,png,gif'
            ]);

            unlink(public_path('uploads/'.$home_ad_data->above_search_ad1));

            $ext = $request->file('above_search_ad1')->extension();
            $final_name = 'above_search_ad1'.'.'.$ext;
            $request->file('above_search_ad1')->move(public_path('uploads/'),$final_name);

            $home_ad_data->above_search_ad1 = $final_name;
        }

        if($request->hasFile('above_footer_ad1')) {
            $request->validate([
                'above_footer_ad1' => 'image|mimes:jpg,jpeg,png,gif'
            ]);

            unlink(public_path('uploads/'.$home_ad_data->above_footer_ad1));

            $ext = $request->file('above_footer_ad1')->extension();
            $final_name = 'above_footer_ad1'.'.'.$ext;
            $request->file('above_footer_ad1')->move(public_path('uploads/'),$final_name);

            $home_ad_data->above_footer_ad1 = $final_name;
        }
        
        if($request->hasFile('above_search_ad2')) {
            $request->validate([
                'above_search_ad2' => 'image|mimes:jpg,jpeg,png,gif'
            ]);

            unlink(public_path('uploads/'.$home_ad_data->above_search_ad2));

            $ext = $request->file('above_search_ad2')->extension();
            $final_name = 'above_search_ad2'.'.'.$ext;
            $request->file('above_search_ad2')->move(public_path('uploads/'),$final_name);

            $home_ad_data->above_search_ad2 = $final_name;
        }

        if($request->hasFile('above_footer_ad2')) {
            $request->validate([
                'above_footer_ad2' => 'image|mimes:jpg,jpeg,png,gif'
            ]);

            unlink(public_path('uploads/'.$home_ad_data->above_footer_ad2));

            $ext = $request->file('above_footer_ad2')->extension();
            $final_name = 'above_footer_ad2'.'.'.$ext;
            $request->file('above_footer_ad2')->move(public_path('uploads/'),$final_name);

            $home_ad_data->above_footer_ad2 = $final_name;
        }
        ...
        $home_ad_data->above_search_ad1_url = $request->above_search_ad_url;
        $home_ad_data->above_search_ad1_status = $request->above_search_ad_status;
        $home_ad_data->above_footer_ad1_url = $request->above_footer_ad_url;
        $home_ad_data->above_footer_ad1_status = $request->above_footer_ad_status;
        $home_ad_data->above_search_ad2_url = $request->above_search_ad_url;
        $home_ad_data->above_search_ad2_status = $request->above_search_ad_status;
        $home_ad_data->above_footer_ad2_url = $request->above_footer_ad_url;
        $home_ad_data->above_footer_ad2_status = $request->above_footer_ad_status;
        $home_ad_data->update();
    ...
    public function top_ad_update(Request $request)
    {
        ...
        if($request->hasFile('top_ad1')) {
            $request->validate([
                'top_ad1' => 'image|mimes:jpg,jpeg,png,gif'
            ]);

            unlink(public_path('uploads/'.$top_ad_data->top_ad1));

            $ext = $request->file('top_ad1')->extension();
            $final_name = 'top_ad1'.'.'.$ext;
            $request->file('top_ad1')->move(public_path('uploads/'),$final_name);

            $top_ad_data->top_ad1 = $final_name;
        }

        if($request->hasFile('top_ad2')) {
            $request->validate([
                'top_ad2' => 'image|mimes:jpg,jpeg,png,gif'
            ]);

            unlink(public_path('uploads/'.$top_ad_data->top_ad2));

            $ext = $request->file('top_ad2')->extension();
            $final_name = 'top_ad2'.'.'.$ext;
            $request->file('top_ad2')->move(public_path('uploads/'),$final_name);

            $top_ad_data->top_ad2 = $final_name;
        }
        
        $top_ad_data->top_ad_url = $request->top_ad_url;
        $top_ad_data->top_ad_status = $request->top_ad_status;
        $top_ad_data->top_ad1_url = $request->top_ad1_url;
        $top_ad_data->top_ad1_status = $request->top_ad1_status;
        $top_ad_data->top_ad2_url = $request->top_ad2_url;
        $top_ad_data->top_ad2_status = $request->top_ad2_status;
        $top_ad_data->update();
